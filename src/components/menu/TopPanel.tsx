...
export default function TopPanel({ menuOpen, onToggleMenu, onClear, onExport }: TopPanelProps) {
  const { createNewThread, removeCurrentThread } = useThreadStore();
  return (
      ...
      <div className="relative">
        ...
        {menuOpen && (
          <div className="absolute left-0 top-full mt-2 w-[130px] px-0.5 pt-2 pb-2 z-50 rounded-lg bg-[#fefefe] shadow-xl shadow-black/5 border border-neutral-200 backdrop-blur-sm overflow-visible flex flex-col">
            <button onClick={async () => { await removeCurrentThread(); onClear(); }} className="w-full flex justify-between text-left px-3 py-2 text-sm hover:bg-neutral-100 flex items-center gap-2 rounded-md cursor-pointer">
            ...
            <button onClick={createNewThread} className="w-full flex justify-between text-left px-3 py-2 text-sm hover:bg-neutral-100 flex items-center gap-2 rounded-md cursor-pointer">
            ...
            <div className="relative group">
              ...
              <div className="absolute left-full -ml-px top-0 p-1 hidden group-hover:block">
                <ThreadsList />
              </div>
            </div>
          </div>
